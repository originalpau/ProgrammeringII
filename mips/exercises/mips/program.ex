defmodule Program do
  ...
  def load({:prgm, code, dataSpace}) do
    #data : [{8,<<0,0,0,0>>},{4,<<0,0,0,0>>},{0,<<0,0,0,0>>}]
    #labels: []
    {data, labels} = endata(dataSpace)
    #IO.puts("data" <> to_string(data))
    :io.format("data : ~w\n\labels: ~w\n\n", [data, labels])

    #code : {code,{<<32,1,0,10>>,<<32,2,0,5>>,<<0,34,24,32>>,<<172,3,0,8>>,<<140,4,0,8>>,<<32,5,0,1>>,<<0,133,32,34>>,<<248,128,0,0>>,<<20,128,255,244>>,<<252,0,0,0>>}}
    #data: {data,#{0 => <<0,0,0,0>>,4 => <<0,0,0,0>>,8 => <<0,0,0,0>>}}
  ...
  def endata([], _, bytes, labels) do {Enum.reverse(bytes), labels} end

  def endata([spec|descr], n, bytes, labels) do
    :io.format("spec: ~w, descr: ~w\nn: ~w\nbytes: ~w\n\labels: ~w\n\n", [spec, descr, n, bytes, labels])
  ...
  def spaces(i, n, bytes) do
    ...
    a = i + rem(4-rem(i,4), 4)
    :io.format("a : ~w\n\n", [a])
    #:lists.seq(from, to, incre)
    #Lists.foldr(list, accumulator, function)
