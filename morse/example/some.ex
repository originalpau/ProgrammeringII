defmodule Morse do
  ...
  def encode([char|text], table, sofar) do
      case sofar do
        [] -> encode(text, table, encode_lookup(char, table))
        _ -> encode(text, table, encode_lookup(char, table) ++ [32] ++ sofar)
      end
  end
  ...
  def decode([], final, _) do Enum.reverse(final) end
  def decode(signal, progress, table) do
    {char, rest} = decode_char(signal, table)
    decode(rest, [char] ++ progress, table)
  end
